...
function statGen(role, level, stamina) {
  ...
  // ------- MINIMUM SYSTEM -------
  // to prevent some stats from being 0
  const minValue = 1;
...
export default function generateCharacter() {
  ...
  const initialMaxStamina = Math.max(40, Math.min(Math.floor(Math.random() * level + 1) * 20, 100)); // higher lvl has hight stamina probability
  const stats = statGen(role, level, initialMaxStamina);
  const character = {
    name: name,
    role: role,
    level: level,
    ...
    xp: 0,
    stats: stats,
    totalPoints: Object.values(stats).reduce((acc, stat) => (acc += stat), 0),
